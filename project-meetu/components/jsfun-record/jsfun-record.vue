	...
	export default {
		...
		onReady() {
			...
			query.select('.canvas').boundingClientRect(data => {
				...
				console.log(data)
				_this.tempw = data.width; //使用canvas的宽度计算中心点的位置
				_this.temph = data.height;
			}).exec();
			query.select('.recording').boundingClientRect(data => {
				// console.log("得到布局位置信息" + JSON.stringify(data));
				// console.log("节点离页面顶部的距离为" + data.top);
				console.log(data)
				_this.tempw1 = data.width; //使用点按图形的宽度计算圆环的宽高
			}).exec();
		},
		methods: {
			moveHandle() {
			...
			record: function() {
				...
				_this.drawObj = setInterval(function() {
					...
					_this.canvasObj.setLineWidth(3);
					_this.canvasObj.arc(centerX, centerY, yuanhuanW, -0.5 * Math.PI, (angle += 2 / (_this.maxTime * 1000 / _this.frame)) *
						Math.PI, false);
					_this.canvasObj.stroke();
